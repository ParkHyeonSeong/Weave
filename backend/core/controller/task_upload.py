...
UPLOAD_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'uploads', 'task')
ALLOWED_EXTENSIONS = {'.jpg', '.jpeg', '.png', '.gif', '.webp'}
MAX_FILE_SIZE = 10 * 1024 * 1024  # 10MB


async def upload_image(branch_id: int, file: UploadFile, request: Request, db: AsyncSession):
    """Task 이미지 업로드"""
    ...
    # 고유 파일명
    filename = f"t{branch_id}_{uuid.uuid4().hex[:12]}{ext}"
    filepath = os.path.join(UPLOAD_DIR, filename)
    with open(filepath, 'wb') as f:
        f.write(content)

    image_url = f"/api/uploads/task/{filename}"
