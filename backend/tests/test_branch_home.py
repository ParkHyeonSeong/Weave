...
async def _make_task(db, branch_id, created_by, status="todo", due_date=None, sprint_id=None):
    ...
    res = await db.execute(text("""
        ...
        VALUES (:b, :dn, :t, :s, :d, :sp, :u) RETURNING task_id
    """), {"b": branch_id, "dn": dn, "t": f"task {dn}", "s": status,
           "d": due_date, "sp": sprint_id, "u": created_by})
...
async def test_find_accessible_enriches_branch_cards(db_session):
    ...
    # two active sprints; Sprint B is created last (higher created_at/sprint_id)
    # so it must win the "most recent active sprint" tiebreak deterministically
    await _make_sprint(db_session, bid, owner, name="Sprint A", status="active")
    await _make_sprint(db_session, bid, owner, name="Sprint B", status="active")

    # 4 tasks: 1 done, 1 cancelled (=> 2 done), 2 incomplete => total 4, 50%
    await _make_task(db_session, bid, owner, status="done")
    ...
    assert b["task_total"] == 4
    assert b["task_done"] == 2
    assert b["progress_percent"] == 50
    assert b["active_sprint_name"] == "Sprint B"  # most recently created active sprint
...
async def test_find_accessible_zero_tasks_progress_is_zero(db_session):
    ...
    branches = await branch_model.find_accessible(owner, db_session)
    b = branches[0]
    assert b["task_total"] == 0
    assert b["task_done"] == 0
