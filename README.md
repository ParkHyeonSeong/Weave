...
- **149 tools** across branches & members (invite/role/remove on every app), tasks (assignees, archive, page links), comments, issues (+ comments), dependencies, sprints, epics, branch config CRUD (statuses, types, labels, custom fields), Canvas docs (pages & annotations), tracks (items & links), schedule (+ task links), scrum (boards & lifecycle, daily/retro cells), search, stars, identity, home KPIs, and notifications — see [`mcp/README.md`](mcp/README.md) for the full list
