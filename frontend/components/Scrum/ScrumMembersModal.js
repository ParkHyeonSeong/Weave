...
import { Users, UserPlus, X, Search, LogOut } from 'lucide-react';
...
export default function ScrumMembersModal({ boardId, myRole, onClose, onChanged }) {
      ...
      <div className="ScrumMembers" onClick={(e) => e.stopPropagation()}>
        <header className="ScrumMembers__Head">
          <div className="ScrumMembers__HeadTitle">
            <Users size={16} />
            <span>멤버</span>
            <em className="ScrumMembers__Count">{members.length}</em>
          </div>
          <button
            type="button"
